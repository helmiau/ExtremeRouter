...
export async function exportDb() {
  ...
  const out = {
    ...
    combos: db.all(`SELECT * FROM combos`).map((r) => ({ id: r.id, name: r.name, kind: r.kind, models: parseJson(r.models, []), strategyConfig: parseJson(r.strategyConfig, {}), revision: Number(r.revision) || 1, createdAt: r.createdAt, updatedAt: r.updatedAt })),
    modelAliases: {},
    customModels: [],
    mitmAlias: {},
    pricing: {},
...
export async function importDb(payload) {
  ...
  db.transaction(() => {
    ...
    for (const c of payload.combos || []) {
      db.run(
        `INSERT OR REPLACE INTO combos(id, name, kind, models, strategyConfig, revision, createdAt, updatedAt) VALUES(?, ?, ?, ?, ?, ?, ?, ?)`,
        [c.id, c.name, c.kind || null, stringifyJson(c.models || []), stringifyJson(c.strategyConfig || {}), Number(c.revision) || 1, c.createdAt || new Date().toISOString(), c.updatedAt || new Date().toISOString()]
      );
    }
  ...
  // Restore replaced the DB wholesale — drop any in-memory caches so the next
  // read reflects the imported data, not the pre-import snapshot.
  invalidateSettingsCache();
  invalidateConnectionsCache();

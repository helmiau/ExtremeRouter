/**
 * SSRF guard on POST /api/cli-tools/cowork-mcp-tools (9router #3782).
 *
...
describe("cowork-mcp-tools SSRF guard", () => {
  ...
  it("rejects cloud-metadata endpoints from remote callers", async () => {
    const res = await POST(remoteRequest("http://169.254.169.254/latest/meta-data"));
    expect(res.status).toBe(400);
  });

  it("allows public URLs from remote callers through to probeMcp", async () => {
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response("{}", { status: 401 })
    );
    const res = await POST(remoteRequest("https://mcp.example.com/sse"));
    // probeMcp ran (fetch called); 401 from the mock surfaces as the auth signal.

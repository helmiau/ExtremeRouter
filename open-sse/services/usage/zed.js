 ...
 * Auth: Authorization: {user_id} {access_token}   (via shared/zedAuth.js)
 *
 * Quota rows are derived from plan.usage (edit_predictions, optional
 * model_requests) and subscription_period.ended_at for billing-cycle reset.
 *
 * LOCAL ADAPTATION vs the 9router original: this gateway stores the
 * long-lived Zed USER token in connection.refreshToken (or
 * providerSpecificData.zedAccessToken) — connection.accessToken holds the
 * short-lived LLM token. buildZedUserAuthHeader picks the user token, so the
 * handler forwards refreshToken through to fetchZedAuthenticatedUser.
 */
...
/**
 * @param {string|null|undefined} accessToken - short-lived LLM token (unused for auth)
 * @param {object|null|undefined} providerSpecificData - { userId, zedAccessToken, systemId }
 * @param {object|null|undefined} proxyOptions
 * @param {string|null|undefined} refreshToken - the long-lived Zed USER token
 */
export async function getZedUsage(
  ...
  const userToken = refreshToken || psd.zedAccessToken || accessToken;
